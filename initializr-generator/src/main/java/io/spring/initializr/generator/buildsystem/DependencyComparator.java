...
public class DependencyComparator implements Comparator<Dependency> {
    ...
    public int compare(Dependency o1, Dependency o2) {
        if (isProjectModuleDependency(o1) && isProjectModuleDependency(o2)) {
            return o1.getArtifactId().compareTo(o2.getArtifactId());
        }

        if (isProjectModuleDependency(o1)) {
            return -1;
        }

        if (isProjectModuleDependency(o2)) {
            return 1;
        }
    ...
    private boolean isProjectModuleDependency(Dependency dependency) {
        String version = dependency.getVersion().getValue();
        return null != version && version.startsWith("${project.parent.version}");
    }
